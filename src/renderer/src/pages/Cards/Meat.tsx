...
const Meat = () => {
    ...
    const [deleteCmds, setDeleteCmds] = useState("");
    const [pointCode, setPointCode] = useState("");
    const [networkDomain, setNetworkDomain] = useState("itui");
    ...
    const generateCmds = () => {
        ...
        let domainKey = domain.get(networkDomain);
...
ent-ip-host:host=ipsg${cardLoc}a:ipaddr=${ipAddress}:type=local
ent-ip-host:host=meat${cardLoc}a:ipaddr=${remoteIpAddress}:type=remote
chg-ip-card:loc=${cardLoc}:defrouter=${defrouter}

ent-dstn:dpc${domainKey}=${pointCode}
ent-ls:lsn=ls${cardLoc}a:lst=a:apc${domainKey}=${pointCode}:adapter=m3ua:maxslktps=12000:ipsg=yes:rsvdslktps=12000:tpsalmtype=maxslktps
ent-assoc:aname=assoc${cardLoc}a:lhost=${ipAddress}:rhost=${remoteIpAddress}:lport=3400:rport=3400:adapter=m3ua:bufsize=200
ent-slk:loc=${cardLoc}:port=a:slc=0:lsn=ls${cardLoc}a:aname=assoc${cardLoc}a
ent-rte:dpc${domainKey}=${pointCode}:lsn=ls${cardLoc}a:rc=10
chg-assoc:aname=assoc${cardLoc}a:open=yes
chg-ls:lsn=ls${cardLoc}a:rcontext=10
alw-card:loc=${cardLoc}
act-slk:loc=${cardLoc}:port=a
`;
        setGeneratedCmds(cmds);

        let deleteCmds = `dact-slk:loc=${cardLoc}:port=a
inh-card:loc=${cardLoc}
chg-assoc:aname=assoc${cardLoc}a:open=no
dlt-rte:dpc${domainKey}=${pointCode}:lsn=ls${cardLoc}a
dlt-slk:loc=${cardLoc}:port=a
dlt-assoc:aname=assoc${cardLoc}a
dlt-ls:lsn=ls${cardLoc}a
dlt-dstn:dpc${domainKey}=${pointCode}
chg-ip-card:loc=${cardLoc}:defrouter=0.0.0.0
dlt-ip-host:host=ipsg${cardLoc}a
dlt-ip-host:host=meat${cardLoc}a
chg-ip-lnk:port=a:loc=${cardLoc}:ipaddr=0.0.0.0
dlt-card:loc=${cardLoc}
`
        setDeleteCmds(deleteCmds);
    ...
    return (
        <div className="flex flex-col p-8 w-full max-w-full h-full overflow-y-auto">
            ...
            <form className="mt-5">
                ...
                <div className="flex gap-4 mt-4 items-center">
                    <div className="w-full">
                        <label className="block font-medium text-sm mb-2">Domain</label>
                        <select
                            className="w-full mb-1 p-2 rounded border border-pink-300"
                            name="network_domain"
                            value={networkDomain}
                            onChange={(e) => {
                                setNetworkDomain(e.target.value);
                            }}
                        >
                            <option value="">Select Network Domain</option>
                            <option value="itui">ITUI</option>
                            <option value="ansi">ANSI</option>
                            <option value="itun">ITUN</option>
                        </select>
                    </div>

                    {inputField("Point Code", pointCode, setPointCode, "point_code", "")}
                    <button
                        type="button"
            ...
            <div className="p-3 bg-pink-50 rounded-lg w-full m-auto shadow mt-10">
                <div className="flex items-center justify-between mb-2">
                    <div>
                        <h2 className="text-lg font-semibold text-pink-900 mb-1">Card Delete Commands:</h2>
                    </div>
                    <button
                        className="cursor-pointer text-pink-800/70 px-3 text-xl py-1 rounded active:text-pink-800/50 transition-colors mb-4"
                        onClick={() => {
                            navigator.clipboard.writeText(deleteCmds);
                        }}
                    >
                        <FaCopy />
                    </button>
                </div>
                <CodeEditor value={deleteCmds} />
            </div>
        </div>
    )
}
